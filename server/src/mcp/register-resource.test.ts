...
describe("registerReadingResource", () => {
  it("keeps current and legacy component URIs available with compatible CSP metadata", async () => {
    ...
    expect(READING_NEST_URI).toBe(
      "ui://ss-reading-nest/app-v88-clear-thought-writeback.html"
    );
    ...
    expect(uri).toBe("ui://ss-reading-nest/app-v88-clear-thought-writeback.html");
    ...
    expect(probe.contents[0].text).toContain("信息库 App 组件已显示");
